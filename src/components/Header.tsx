...
export default function Header() {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 bg-white border-b border-stone-100 shadow-sm" id="main-header">
        {/* Top Main Navbar */}
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-3 sm:py-4 flex items-center justify-between gap-2.5 sm:gap-4">
          ...
          <div 
            ...
            id="brand-logo"
          >
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-pastel-pink flex items-center justify-center border-2 border-brand-200 shadow-sm group-hover:scale-105 transition-transform duration-300">
              <span className="text-sm sm:text-xl">🌸</span>
            </div>
            <div className="leading-tight">
              <h1 className="text-base sm:text-2xl font-black tracking-tight text-brand-500 flex items-center gap-1 font-display">
                Yarnova
              </h1>
              <p className="text-[9px] uppercase tracking-wider text-stone-400 font-extrabold -mt-0.5 hidden min-[380px]:block">Handmade Magic</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-3" id="header-actions">
            ...
            <button
              ...
              className="p-1.5 text-stone-600 hover:text-brand-500 hover:bg-pastel-pink/30 rounded-full transition-all relative shrink-0"
              title="Wishlist"
            ...
            {user ? (
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-1.5 bg-[#fbfbf8] hover:bg-stone-100 border border-stone-200 text-stone-700 font-medium text-xs sm:text-sm py-1.5 px-3 rounded-full transition-all shrink-0"
              ...
              <button
                onClick={() => setView('account')}
                className="flex items-center justify-center gap-1.5 bg-brand-500 hover:bg-brand-600 text-white text-xs sm:text-sm font-semibold p-2 sm:py-2 sm:px-4 rounded-full transition-all shadow-sm shadow-brand-200 shrink-0"
                id="header-login-btn"
                title="Join Nest"
              >
                <UserIcon size={16} />
                <span className="hidden min-[450px]:inline">Join Nest</span>
              </button>
            )}
          </div>
        </div>
      ...
      {/* Mobile Menu Slideout - Rendered outside <header> to avoid parent stacking/backdrop-filter bugs */}
      <AnimatePresence>
        {mobileMenuOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-0 bg-stone-900/60 z-[9998]"
              id="mobile-menu-backdrop"
            />
            
            {/* Slideout drawer with absolutely solid background */}
            <motion.div
              initial={{ x: '-100%' }}
              animate={{ x: 0 }}
              exit={{ x: '-100%' }}
              transition={{ type: 'tween', duration: 0.3 }}
              className="fixed inset-y-0 left-0 w-4/5 max-w-sm shadow-2xl z-[9999] p-6 flex flex-col justify-between border-r border-stone-200"
              ...
              <div>
                <div className="flex items-center justify-between pb-6 border-b border-stone-100">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xl font-black font-display text-brand-500">Yarnova</span>
                  </div>
                  <button
                    onClick={() => setMobileMenuOpen(false)}
